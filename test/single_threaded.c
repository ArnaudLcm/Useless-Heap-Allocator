...
void test_valid_chunk_alloc() {
    ...
    ASSERT_TRUE_FATAL(first_alloc != NULL);
    ASSERT_TRUE(first_alloc == local_arena->arena_start + sizeof(chunk_metadata_t));

    void* new_alloc = alloc(1 << 4);

    ASSERT_TRUE(new_alloc == local_arena->arena_start + sizeof(chunk_metadata_t) + (1 << 4) + sizeof(chunk_metadata_t));
}
...
void test_valid_chunk_alloc_and_dealloc() {
    ...
    void* new_alloc = alloc(1 << 4);
    ASSERT_TRUE_FATAL(new_alloc != NULL);
...
void test_valid_chunk_alloc_and_resize_with_copy() {
    ...
    *second_alloc = 1234;
    ...
    ASSERT_TRUE_FATAL(new_alloc != NULL);
    ASSERT_TRUE_FATAL(second_alloc != NULL);
    ASSERT_TRUE_FATAL(third_alloc != NULL);
...
void test_alloc_chunk_with_coalesce() {
    ...
    ASSERT_TRUE_FATAL(second_alloc != NULL);
    ASSERT_TRUE_FATAL(third_alloc != NULL);
    ASSERT_TRUE_FATAL(fourth_alloc != NULL);
